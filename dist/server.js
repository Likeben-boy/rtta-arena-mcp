...
"检查当前钱包的地址和 USDC 余额。在采取行动前使用此工具验证会话是否处于活跃状态。", {}, // 无需参数
async () => {
    // 检查钱包是否已初始化
    if (!playerWallet) {
        return {
...
"启动一个自主的后台游戏循环，自动投票、聊天、结算轮次和领取奖励。立即返回 — 使用 get_auto_play_status 监控进度。", {
    ...
    const config = {
        roomId,
        voteStrategy: voteStrategy ?? DEFAULT_CONFIG.voteStrategy, // 投票策略
        chatStrategy: chatStrategy ?? DEFAULT_CONFIG.chatStrategy, // 聊天策略
...
// ============ 工具 10: 停止自动玩 ============
server.tool("stop_auto_play", // 工具名称
"停止正在运行的自动玩游戏循环并返回最终统计信息。", {}, // 无需参数
async () => {
    // 检查是否有活跃的游戏循环
    if (!activeGameLoop) {
        return {
            content: [{ type: "text", text: "No active auto-play loop to stop." }],
        };
    }
    // 停止游戏循环并获取最终状态
...
// ============ 工具 11: 获取自动玩状态 ============
server.tool("get_auto_play_status", // 工具名称
"检查当前自动玩游戏循环的进度：轮次、人性分、已投票数、已发送消息数、错误数。", {}, // 无需参数
async () => {
    // 检查是否有活跃的游戏循环
    if (!activeGameLoop) {
        return {
            content: [{ type: "text", text: "No active auto-play loop. Use auto_play to start one." }],
        };
    }
    // 获取游戏循环的当前状态
...
"创建一个新的游戏房间。你成为房间创建者并自动加入（收取入场费）。Tier 控制游戏节奏：Quick (0) = 快速轮次，Standard (1) = 平衡，Epic (2) = 长游戏。返回新房间 ID。", {
    ...
    try {
        ...
        const feeWei = BigInt(entryFee) * 1000000n;
...
// ============ 工具 14: 铸造测试 USDC ============
server.tool("mint_test_usdc", // 工具名称
"向你的钱包铸造测试 USDC（仅适用于本地 Anvil 或带有 MockUSDC 的测试网）。用于在加入游戏前为你的机器人提供资金。", {
    ...
    try {
        ...
        // 将 USDC 金额转换为 Wei（USDC 有 6 位小数）
        const amountWei = BigInt(amount) * 1000000n;
        // 调用代币的 mint 函数铸造代币
        ...
        if (msg.includes("execution reverted") || msg.includes("is not a function")) {
            ...
            return {
                content: [{ type: "text", text: "Error: mint() failed. This only works with MockUSDC on local/test networks." }],
                isError: true,
            };
        }
        // 其他类型的错误
